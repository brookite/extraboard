...
export const en = {
	...
	propertyBadges: {
		...
		timeFrom: 'From',
		timeTo: 'To',
		clearEndTime: 'Clear the end time',
		setDate: 'Set date',
		addDate: 'Add',
	...
	list: {
		...
		moveStackUp: 'Move stack up',
		moveStackDown: 'Move stack down',
		groupBy: {
			label: 'Group by',
			section: 'Sections',
			stack: 'Stacks',
			chipSection: 'Sections',
			chipStack: 'Stacks',
			desc: 'What the list gathers its cards into.',
		},
		...
		sort: {
			...
			ascShort: 'Earliest first',
			descShort: 'Latest first',
			asc: '{name} (earliest first)',
			desc: '{name} (latest first)',
			noDateProperty: 'This board declares no date property',
		},
		filter: {
			label: 'Filter by tag',
			clear: 'Clear filter',
			noTags: 'No tags in this section',
		},
	},
	divider: {
	...
	filter: {
		...
		emptyGroup: 'No conditions — this group matches everything.',
	...
	// The quick view settings panel (views.md §5).
	viewOptions: {
		title: 'View options',
	...
	// Which value a drag edits when a card sits on the grid through more than
	// one property (calendar-view.md §6.2).
	dateProperty: {
		moveTitle: 'Which date should move?',
		clearTitle: 'Which date should be cleared?',
		setTitle: 'Which date should be set?',
		moveMessage: 'This card is on that day through several properties.',
		setMessage: 'This calendar is computed from several properties.',
		all: 'All of them',
		notSet: 'not set',
	},
	recurrenceText: {
	...
	modal: {
		...
		views: {
			...
			groupByDesc: 'Sections read the board across its dividers; stacks read it column by column.',
			controls: {
				label: 'Section controls',
				desc: 'Where each section\u2019s sort and filter live.',
				dynamic: 'Saved per section',
				fixed: 'Fixed for the whole view',
				session: 'Reset when the view is left',
			},
			listSort: 'Sort',
			listSortDesc: 'Applied to every section of this view.',
			listTags: 'Tag filter',
			listTagsDesc: 'Comma separated. A card is shown when it carries any of them.',
			calendar: 'Calendar',
			type: 'Type',
			...
			filtersDesc: 'Conditions and sort keys for this list.',
			dateProperties: 'Date properties',
			datePropertiesDesc:
				'Cards are placed on the grid by these properties. A card dated by several of them is shown once per date, and once only where those dates share a cell.',
			mode: {
				label: 'Mode',
				month: 'Month',
				week: 'Week',
		...
		boardSettings: {
			defaultView: {
				name: 'Default view',
				desc: 'What the board opens as. More views can be added at any time.',
			},
	...
	notice: {
		notABoard: 'Not a board: add an "extraboard" property to the note frontmatter.',
	...
	propertyDefs: {
		...
		diagnostic: {
			...
			timespanNeedsTime: 'Property "{name}": time ranges need a time to be allowed.',
